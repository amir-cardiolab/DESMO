...
r = 2
r_DESMO = 2
save_modes_path = f'./figures/DESMOFourier_ICA_r{r_DESMO}/modes/'
...
# Define the x values to span the period [0, 60]
t_points = torch.linspace(0, m, m)

def fourier_series(x, period, *coeffs):
    # Number of terms in the Fourier series
    n_terms = (len(coeffs) - 1) // 2
    period = period.to(device)
    x = x.to(device)
    # The first coefficient (a0) is a constant offset
    a0 = coeffs[0]
    
    # Initialize the result with the first term (a0)
    result = a0 * torch.ones_like(x).to(device)
    
    # Loop over the rest of the Fourier terms
    for n in range(1, n_terms + 1):
        an = coeffs[2 * n - 1]
        bn = coeffs[2 * n]
        
        # Compute the Fourier terms and add to the result
        result += an * torch.cos(2 * torch.pi * n * x / period).to(device) + bn * torch.sin(2 * torch.pi * n * x / period).to(device)
...
# %%
class DESMOFourier(nn.Module):
    def __init__(self, n, m, polyorder, r_SINDyAE, omega_init=10000, nF = 10):
        super(DESMOFourier, self).__init__()

        # Optimizable spatial modes
        self.phi_list = nn.ParameterList([nn.Parameter(torch.ones(n)) for _ in range(r_SINDyAE)])

        # Calculate the number of polynomial terms for r variables of order polyorder
        num_terms = calculate_number_of_terms(r_SINDyAE, polyorder)
        print('Number of terms in polynomial library:', num_terms)

        # Create vector of optimizable coefficients for sparsity
        self.c_coef = nn.Parameter(torch.ones(num_terms))

        # Optimizable temporal coefficients for polynomial terms
        self.z_list = nn.ParameterList([nn.Parameter(torch.ones(2 * nF + 1)) for _ in range(num_terms)])
        self.period_list = nn.ParameterList([nn.Parameter(torch.ones(1)*period_init) for _ in range(num_terms)])
        self.trig_period_list = nn.ParameterList([nn.Parameter(torch.ones(1)*period_init) for _ in range(3* r_SINDyAE)])

        # Temporal coefficients for sin, cos, and tanh terms
        self.zsin_list = nn.ParameterList([nn.Parameter(torch.ones(2 * nF + 1)) for _ in range(r_SINDyAE)])
        self.zcos_list = nn.ParameterList([nn.Parameter(torch.ones(2 * nF + 1)) for _ in range(r_SINDyAE)])
        self.ztanh_list = nn.ParameterList([nn.Parameter(torch.ones(2 * nF + 1)) for _ in range(r_SINDyAE)])

        # Sparsity coefficients for sin, cos, and tanh terms
        self.sin_coef_list = nn.ParameterList([nn.Parameter(torch.tensor(1.0)) for _ in range(r_SINDyAE)])
        self.cos_coef_list = nn.ParameterList([nn.Parameter(torch.tensor(1.0)) for _ in range(r_SINDyAE)])
        self.tanh_coef_list = nn.ParameterList([nn.Parameter(torch.tensor(1.0)) for _ in range(r_SINDyAE)])

        # Define optimizable frequencies for sin, cos, and tanh terms
        self.omega_list = nn.ParameterList([nn.Parameter(torch.tensor(1.0)*omega_init) for _ in range(3 * r_SINDyAE)])
    ...
    def forward(self, X):
        ...
        # Create candidate library for spatial modes
        theta_phi = self.c_coef * POOL_DATA(latent_spatial, len(self.phi_list), polyorder)

        z_values = torch.stack([fourier_series(t_points, period, *z) for period, z in zip(self.period_list, self.z_list)], dim=0)

        # Fourier terms: sin, cos, and tanh contributions
        fourier_contrib = 0
        for i in range(len(self.phi_list)):
            phi = phi_list[i]
            zsin = fourier_series(t_points, self.trig_period_list[3*i], *self.zsin_list[i])
            zcos = fourier_series(t_points, self.trig_period_list[3*i+1], *self.zcos_list[i])
            ztanh = fourier_series(t_points, self.trig_period_list[3*i+2], *self.ztanh_list[i])
...
polyorder = 2
# Redo POD if more modes are needed for DESMO initialization
if r_DESMO < r:
    r = r_DESMO
    X_approx, POD_modes, temporal_coeffs, S = POD_analysis(X,plot_POD_flag, x_range, y_range, z_range, r)
omega_init = 10000
period_init = 1000
nFourier = 30 # number of Fourier_terms
model_desmo = DESMOFourier(n, m, polyorder,r_DESMO,omega_init, nFourier).to(device)
...
optimizer = torch.optim.Adamax([
    ...
    # Period parameters group
    {'params': [param for param in model_desmo.period_list] +
               [param for param in model_desmo.trig_period_list],
     'lr': 1e-2},
], weight_decay=0.0)

scheduler_temporal = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', patience=500, factor=0.1, verbose=True,min_lr=1e-6)
criterion = nn.MSELoss()
# Model info
pytorch_total_params = sum(p.numel() for p in model_desmo.parameters())
print("Total number of parameters in DESMO:", pytorch_total_params, flush = True)


# In[10]:


def poly_norm(c_coef,z,phi,period):
    ...
    period_list =[param.data for param in period]
    z_list = [param.data for param in z]
    zs = torch.stack([fourier_series(t_points, period, *z) for period, z in zip(period_list, z_list)], dim=0)
...
def nonlinear_norm(sin_coef,cos_coef,tanh_coef,zsin,zcos,ztanh,phi,omega,period):
    ...
    trig_period_list =[param.data for param in period]
    ...
    for i in range(phis.shape[1]):
        ...
        zsin = fourier_series(t_points, trig_period_list[3*i], *zsin_list[i])
        zcos = fourier_series(t_points, trig_period_list[3*i+1], *zcos_list[i])
        ztanh = fourier_series(t_points, trig_period_list[3*i+2], *ztanh_list[i])
...
for epoch in range(num_epochs):
    ...
    if epoch % 10 == 0:
        print(f'Epoch [{epoch+1}/{num_epochs}], Rec Loss: {loss.item():.8f}, Spatial ortho loss: {ortho_loss_spatial.item():.8f}, L1 loss: {l1_loss.item():.8f} ', flush = True)
    scheduler_temporal.step(total_loss)
    
    if epoch % 2000 == 0:
        current_time = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        save_path = f'./models/DESMOFourier_r{r_DESMO}_epoch{epoch+1}_{current_time}.pt'
...
plt.ylabel("rec loss")
plt.yscale('log')
os.makedirs(os.path.dirname('./figures/DESMOF/'), exist_ok=True)
plt.savefig('./figures/DESMOF/loss.png')
plt.show()
plt.close()
current_datetime = datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
filename = f'./models/DESMOFourier_r{r_DESMO}_final_{current_datetime}.pt'
...
if plot_DESMO_flag:
    ...
    plt.subplot(3, 2, 1)
    plt.imshow(np.reshape(out_x,(m,x_range,y_range,z_range))[time_to_plot,:, :,plane_to_plot].T, origin='lower')
    plt.title("True")
    plt.colorbar()
    plt.subplot(3, 2, 2)
    plt.imshow(np.reshape(recon_dual,(m,x_range,y_range,z_range))[time_to_plot,:, :,plane_to_plot].T, origin='lower')
    plt.title("DESMO Predicted")
    plt.colorbar()


    plt.subplot(3, 2, 3)
    plt.imshow(np.reshape(latent_spatial,(x_range,y_range,z_range,r_DESMO))[:,:,plane_to_plot, 0].T, origin='lower')
    plt.title("DESMO1")
    plt.colorbar()
    plt.subplot(3, 2, 4)
    plt.imshow(np.reshape(latent_spatial,(x_range,y_range,z_range,r_DESMO))[:,:,plane_to_plot, 1].T, origin='lower')
    plt.title("DESMO2")
    plt.colorbar()

    plt.subplot(3, 2, 5)
    plt.imshow(np.reshape(POD_modes,(x_range,y_range,z_range,r))[:, :,plane_to_plot,0].T, origin='lower')
    plt.title("POD1")
    plt.colorbar()
    plt.subplot(3, 2, 6)
    plt.imshow(np.reshape(POD_modes,(x_range,y_range,z_range,r))[:, :,plane_to_plot,1].T, origin='lower')
    plt.title("POD2")
    plt.colorbar()
...
polynorms = torch.stack(poly_norm(model_desmo.c_coef,model_desmo.z_list,model_desmo.phi_list,model_desmo.period_list))
nlnorms = torch.stack(nonlinear_norm(model_desmo.sin_coef_list,model_desmo.cos_coef_list,model_desmo.tanh_coef_list,
                    model_desmo.zsin_list,model_desmo.zcos_list,model_desmo.ztanh_list,model_desmo.phi_list,
                    model_desmo.omega_list,model_desmo.trig_period_list))
# In[72]:
...
threshold_values = [pow(10,-i) for i in np.arange(5, -5, -0.5)]
