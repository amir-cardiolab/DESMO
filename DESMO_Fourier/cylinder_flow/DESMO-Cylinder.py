...
r = 2
r_DESMO = 2
save_modes_path = f'./figures/DESMOFourier_Cylinder_r{r_DESMO}/modes/'
...
class DESMOFourier(nn.Module):
    def __init__(self, n, m, polyorder,r_DESMO, omega_init = 10000,nF = 10):
        ...
        self.trig_period_list = nn.ParameterList([nn.Parameter(torch.ones(1)*period_init) for _ in range(3* r_DESMO)])

        # Temporal coefficients for sin, cos, and tanh terms
        self.zsin_list = nn.ParameterList([nn.Parameter(torch.ones(2 * nF + 1)) for _ in range(r_DESMO)])
        self.zcos_list = nn.ParameterList([nn.Parameter(torch.ones(2 * nF + 1)) for _ in range(r_DESMO)])
        self.ztanh_list = nn.ParameterList([nn.Parameter(torch.ones(2 * nF + 1)) for _ in range(r_DESMO)])
...
omega_init = 10000
period_init = 60
nFourier = 10 # number of Fourier_terms
...
plt.ylabel("rec loss")
plt.yscale('log')
plt.savefig(save_modes_path + 'Cylinder_loss.png')
plt.show()
plt.close()
current_datetime = datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
save_model_path = f'./models/DESMOCF_r{r_DESMO}_final_{current_datetime}.pt'
os.makedirs(os.path.dirname(save_model_path), exist_ok=True)
torch.save(model_desmo.state_dict(), save_model_path)
...
threshold_values = [pow(10,-i) for i in np.arange(6, -5, -0.5)]
